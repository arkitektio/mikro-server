"""Reading a fabriks collection's ``fabriks.json`` without depending on fabriks.

`fabriks` is the library that *writes* this format. This module reads the one file that says
what was written, and it does so with `json` and nothing else -- the same relationship the
datalayer has with zarr, where ``get_zarr_metadata`` parses ``zarr.json`` by hand and no zarr
package is installed. The reasons are the same three:

**A server that imports a writer inherits its dependencies.** fabriks needs pyarrow, numpy and
a mesh simplifier to do its job. None of that is required to read one small JSON object, and
all of it would have to be installed, pinned and upgraded in a service whose entire interest
in a collection is "is this registerable, and what does it declare".

**A version skew becomes an outage.** If registration went through the writer's parser, a
fabriks release that changed its dataclasses could stop a running server reading collections it
had already accepted. Parsing the wire format directly means the *format* is the contract, and
the writer and the server are two independent implementations of it -- which is also the only
arrangement in which "the format is specified" is a testable claim rather than a shared object
file.

**The reader is not the writer's mirror.** fabriks's ``Manifest`` exists to be *round-tripped*:
it carries defaults, constructors and the vocabulary a writer needs to make choices. What a
server needs is narrower and stricter -- did this thing declare a format I can read, and does
what it declared hang together. Sharing a class would make one of those two jobs the other's
special case.

**What this module deliberately does not do:** open a Parquet file. Everything here comes from
one GET of one small object. Checking the geometry against what the manifest claims is
:mod:`core.logic.meshes`'s job, and it happens later, over DuckDB.
"""
...
#: familiar -- registering a collection nothing can decode is the failure the mesh contract has
#: been fighting since it had two versions.
#:
#: Kept in step with ``fabriks.manifest.SPEC_VERSION`` by the contract, not by an import.
SUPPORTED_VERSIONS = frozenset({"1"})

#: The manifest's name, at the root of the collection's prefix.
MANIFEST_NAME = "fabriks.json"
...
#:
#: ``decimation`` names what a writer actually did rather than offering one name that is
#: sometimes a lie: declaring ``QUARTER`` over a different reduction would be a claim about the
#: geometry that nothing could check. ``boundary: OPEN`` is accepted rather than refused -- the
#: argument against it (an isolated object shows holes where two cells meet at different levels)
#: is a rendering trade-off, and refusing a file the format defines would reject collections
#: fabriks legitimately produces.
ENCODING_VOCABULARY: dict[str, frozenset[str]] = {
    "positions": frozenset({"UINT16_QUANTIZED_PER_CELL"}),
    "indices": frozenset({"UINT16", "UINT32"}),
    "codec": frozenset({"MESHOPT", "NONE"}),
    "compression": frozenset({"NONE", "ZSTD"}),
    "boundary": frozenset({"LOCKED", "OPEN"}),
    "decimation": frozenset({"QUARTER", "HALF", "EIGHTH", "CUSTOM"}),
}

#: The keys a decoder cannot work without, so none of them is ever defaulted on a writer's
#: behalf. ``codec`` and ``compression`` are the load-bearing pair: a wrong guess is not an
#: error anywhere, it is geometry that decodes to garbage.
REQUIRED_ENCODING_KEYS = ("positions", "indices", "codec", "compression", "boundary", "decimation")
...
class Manifest:
    """What one GET of ``fabriks.json`` establishes about a collection.
    ...
    def cell_size(self) -> list[int]:
        """The level-0 cell, one size per component, in the vertex component order."""
...
def _entry_path(raw: Any) -> str:
    ...
    A fabriks-written entry is an object carrying ``bytes`` and ``rowGroups`` alongside the path,
    so a reader can range-read a part without being able to stat it. The server needs only the
    path -- it addresses files, it does not seek inside them -- but it must not choke on the
    richer form.
    """
...
def _assert_grid(grid: Any) -> dict[str, Any]:
    ...
    if not isinstance(cell_size, list) or len(cell_size) != _CELL_SIZE_RANK:
        raise ManifestError(f"`grid.cellSize` is the level-0 cell in voxels, one size per component, so it takes exactly {_CELL_SIZE_RANK} values, got {cell_size!r}.")
    ...
    if not isinstance(levels, int) or isinstance(levels, bool) or levels < 1:
        raise ManifestError(f"`grid.levels` is how many octree levels this collection stores, so it is a whole number of at least 1, got {levels!r}.")
...
def _assert_encoding(encoding: Any) -> dict[str, Any]:
    ...
    # `normals` is optional and, in this format, usually absent: fabriks writes no normals column
    # and a renderer computes vertex normals itself. Carried through when stated so the geometry
    # check can compare it against the file's own columns.
    resolved = {key: str(encoding[key]) for key in REQUIRED_ENCODING_KEYS}
    if encoding.get("normals") is not None:
        resolved["normals"] = str(encoding["normals"])
    return resolved
...
def _assert_axes(axes: Any) -> list[str] | None:
    ...
    **Absent is legitimate and common.** Nothing in the format is decoded through `axes`:
    vertex components, `cellSize`, the bbox columns and the Morton interleave are all positional
    and self-consistent without any names. Naming those axes is a question about the
    collection's relationship to *something else* -- the image it was extracted from, the
    coordinate graph it is placed in -- which is a layer above the bytes.
    ...
    if isinstance(axes, str) or not isinstance(axes, list) or len(axes) != _AXIS_RANK:
        raise ManifestError(f"A mesh collection is three-dimensional, so `axes` names {_AXIS_RANK} axes, got {axes!r}.")
...
def parse_manifest(body: bytes, *, where: str = "the manifest") -> Manifest:
    """Read ``fabriks.json``'s bytes into the facts a registration acts on.
    ...
    if "grid" not in raw or "encoding" not in raw:
        raise ManifestError(f"{where} must carry a `grid` and an `encoding` object: they are how a reader turns a Morton code into a box and the blobs into geometry, and nothing else in the store states them.")
