...
@kante.pydantic_input(
    ...
    description=(
        "Attach metadata spokes to an array dataset or a table dataset after ingest. Exactly one of `dataset` and `table` names the container; `anchor` carries the coordinates "
...
class CreateCoordinateAnchorInput:
    ...
    dataset: strawberry.ID | None = strawberry.field(default=None, description="The array dataset to anchor into. Exactly one of `dataset` and `table`")
    table: strawberry.ID | None = strawberry.field(default=None, description="The table dataset to anchor into. Exactly one of `dataset` and `table`")
    anchor: CoordinateAnchorInput = strawberry.field(
        description=(
            ...
            "axis-typed columns and values along them. An empty `axisAnchors` list is an anchor global over the whole container. Phasor spokes are array-only"
        )
    )
...
def create_coordinate_anchor(info: Info, input: CreateCoordinateAnchorInput) -> types.CoordinateAnchor:
    ...
    if (model.dataset is None) == (model.table is None):
        raise ValueError("Name exactly one of `dataset` or `table`: an anchor pins into one container.")

    with transaction.atomic():
        if model.dataset is not None:
            container: models.ArrayDataset | models.TableDataset = get_for_org(models.ArrayDataset, info, id=model.dataset)
