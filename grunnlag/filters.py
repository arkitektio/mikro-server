...
class ExperimentFilter(django_filters.FilterSet):
    ...
    tags = django_filters.BaseInFilter(label="The tags you want to filter by", field_name="tags__name")



class RepresentationFilter(django_filters.FilterSet):
    tags = django_filters.BaseInFilter(method='my_tag_filter',label="The tags you want to filter by")
    experiments = django_filters.ModelMultipleChoiceFilter(queryset=Experiment.objects.all(), field_name= "sample__experiments", label="The Experiment the Sample of this Representation belongs to")
    ...
    has_metric = django_filters.CharFilter(method='metric_filter',label="Filter by required Metric Keys (seperated by ,)")
    ...
    def thumbnail_filter(self, queryset,name, value):
        if value: return queryset.exclude(thumbnail__isnull=True).exclude(thumbnail__exact="")
        return queryset
...
class RepresentationMetricFilter(django_filters.FilterSet):
    keys = django_filters.BaseInFilter(method='my_key_filter',label="The key you want to filter by")
    sample = django_filters.ModelChoiceFilter(queryset=Sample.objects.all(),field_name= "rep__sample")
    representation = django_filters.ModelChoiceFilter(queryset=Representation.objects.all(),field_name= "rep")
    order = django_filters.BaseInFilter(method='order_filter',label="Order by Keys")
